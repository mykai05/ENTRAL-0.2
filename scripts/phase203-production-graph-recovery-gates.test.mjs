...
test("all later releases require a real authenticated production member journey", async () => {
  ...
  assert.match(journey, /getAttribute\("data-mobile-dimension"\) === expectedDimension/);
  assert.match(journey, /url\.searchParams\.get\("graph"\) === dimension/);
  assert.match(journey, /switchMobileGraphDimension\(page, initialToolbar, "2d"/);
