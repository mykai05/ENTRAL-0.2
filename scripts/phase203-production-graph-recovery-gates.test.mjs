...
test("all later releases require a real authenticated production member journey", async () => {
  ...
  assert.match(contracts, /UNBOUND_MIGRATED_MEMBER_PROVENANCE/);
  assert.match(contracts, /UNBOUND_MEMBER_JOURNEY_DEPLOYMENT/);
  assert.match(contracts, /canonical_endpoint_readback/);
  assert.match(contracts, /rendered_subset_authorized/);
  assert.match(contracts, /destination_sync_errors/);
  ...
  assert.match(journey, /E2E_MIGRATED_STATE_RECEIPT/);
  assert.match(journey, /E2E_DEPLOYMENT_READBACK_RECEIPT_SHA256/);
  assert.match(journey, /E2E_DEPLOYMENT_READBACK_RECEIPT/);
  assert.match(journey, /verifiedReceipt/);
  assert.match(journey, /PHASE_202_MIGRATION_CUTOVER_AT/);
  assert.match(journey, /PHASE_202_MIGRATED_STATE_RECEIPT_SHA256/);
