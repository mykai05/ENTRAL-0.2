...
- A compact seven-node organization neural-map summary on `/member` plus a complete read-only organization graph at `/member/graph`. Both are derived only from the same already authorized overview response; neither queries the internal Command OS or executable agent tables.

The neural map is an organization operating view, not a representation of autonomous execution. The complete view expands the organization core into six operating domains and the records the member is already authorized to see: the latest health assessment, every published priority, up to eight returned member-visible tasks plus a truthful aggregate rollup when more work exists, up to five organization members, every published finding and recommendation, the monthly summary record, and every published accomplishment and next priority. With the current bounded overview contract this is at most 91 nodes. Exact assignee links are created only when a returned task references a returned organization member; the browser cannot supply or substitute an organization identifier.

Every graph node is therefore either a fixed Entral operating domain or a representation of an organization-scoped record already present in `MemberOverviewResponse`. The compact `/member` map remains a deliberately restrained dashboard summary. The complete `/member/graph` route uses Entral's full WebGL command-field visual language: a depth-aware 3D scene, glowing neurons and connections, signal particles, orbital camera controls, a searchable hierarchy, selected-neuron inspector, full-screen mode, and local speed, brightness, particle, label, palette, and per-branch color controls. It does not import or render the internal `NeuronsCommandCenter`, and it has no command input, agent execution, connector, or mutation path.

Selecting a neuron or changing the 3D controls changes only the local rendering. The scene builder accepts only `MemberOverviewResponse`, the already authorized DTO returned for the verified organization, and performs no network request. Color preferences are stored locally under an organization-specific browser key and contain colors only. The complete view makes record counts and source limits explicit instead of implying that truncated API data is exhaustive. The canvas supports pointer orbit, wheel zoom, arrow-key rotation, plus/minus zoom, Home reset, visible focus, and a hierarchy that remains fully usable if WebGL is unavailable. Auto-orbit can be paused, reduced-motion preference pauses it by default, and responsive layouts intentionally move the hierarchy and inspector below the 3D field on narrow screens to preserve touch targets and eliminate horizontal overflow.
