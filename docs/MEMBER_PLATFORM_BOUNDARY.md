...
- A compact seven-node organization neural-map summary on `/member` plus the read-only multi-business Command Universe at `/member/graph`. Both are derived only from the same already authorized overview response; neither queries the internal Command OS or executable agent tables.

The compact neural map is an organization operating view, not a representation of autonomous execution. The complete member graph is the Command Universe: ENTRAL is the fixed center, Marshals own portfolio theaters, Generals represent businesses, Commanders represent departments, and Soldiers represent bounded execution roles. A published organization hierarchy may contain multiple Generals and up to 5,000 sanitized nodes. The WebGL renderer draws at most 900 nodes at once, prioritizing higher command layers and disclosing how many lower-level nodes were omitted from the current rendering.
...
Every graph node is therefore either a sanitized command-hierarchy node, a fixed Entral operating domain, or a representation of an organization-scoped record already present in `MemberOverviewResponse`. The compact `/member` map remains a deliberately restrained dashboard summary. The complete `/member/graph` route uses Entral's full WebGL command-field visual language: a parent-relative solar-system layout, glowing neurons and connections, signal particles, orbital camera controls, a searchable chain of command, selected-neuron inspector, full-screen mode, and local speed, brightness, particle, label, palette, and per-branch color controls. It does not import or render the internal `NeuronsCommandCenter`, and it has no command input, agent execution, connector, or mutation path.
