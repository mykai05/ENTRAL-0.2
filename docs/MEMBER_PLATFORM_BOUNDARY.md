...
Every graph node is therefore either a fixed Entral operating domain or a representation of an organization-scoped record already present in `MemberOverviewResponse`. Selecting a node changes only the local inspector; it performs no network mutation. The complete view makes record counts and source limits explicit instead of implying that truncated API data is exhaustive. Motion is restrained, pauses when the graph or page is not visible, can be paused manually, and honors the browser's reduced-motion preference on first render. The layout uses concentric rings on wide screens and an intentional one- or two-column record list on narrow screens so nodes remain legible, keyboard reachable, and free of horizontal overflow.
